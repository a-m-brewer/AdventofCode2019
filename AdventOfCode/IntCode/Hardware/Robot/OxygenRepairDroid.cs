...
{
    public class OxygenRepairDroid
    {
        private readonly bool _showScreen;
        ...
        public OxygenRepairDroid(UserMode userMode, Dictionary<string, (int visited, OxygenNode node)> seen = null, bool showScreen = false)
        {
            ...
            var funcInputModule = userMode switch
            {
                UserMode.Human => new FuncInputModule(Human),
                UserMode.Random => new FuncInputModule(Rand),
                UserMode.LeastExplored => new FuncInputModule(LeastExplored),
                _ => throw new ArgumentOutOfRangeException(nameof(userMode))
            };
            
            _random = new Random();
            
            _computer = new Computer(funcInputModule, new ActionOutputModule(OutputCallback));
        ...
        public OxygenRepairDroidResult Repair()
        {
            _computer.Load(CreateProgram());
            _computer.Run();
            return new OxygenRepairDroidResult(_seen, _x, _y);
        }
        ...
        private long Human()
        {
            ...
            var retrievedInput = false;
            while (!retrievedInput)
            {
                var parsedInput = long.TryParse(Console.ReadLine(), out _dir);
                retrievedInput = parsedInput && 0 < _dir && _dir < 5;
            }
        ...
        private long Rand()
        {
            ...
            _dir = _random.Next(1, 5);
            return _dir;
        }

        private long LeastExplored()
        {
            if (_showScreen)
            {
                Console.Write(_screen);
            ...
            var nextPositions = _dirs.Select(dir =>
            {
                var nextPos = GetNextPosition(dir);
                var nextKey = GetPosKey(nextPos.x, nextPos.y);
                return new
                {
                    dir,
                    nextPos,
                    visitedCount = _seen.TryGetValue(nextKey, out var visited) ? visited.visited : 0
                };
            }).OrderBy(o => o.visitedCount).ToList();

            var lowestCount = nextPositions.First().visitedCount;
            var nextLowest = nextPositions.Where(w => w.visitedCount == lowestCount).ToList();
            var nextLowestIndex = _random.Next(nextLowest.Count);
            _dir = nextLowest[nextLowestIndex].dir;
        ...
        private void OutputCallback(long val)
        {
            ...
            {
                case 0:
                    _screen.Update(new Pixel {Value = "#", X = nextX, Y = nextY});
                    break;
                case 1:
                    _screen.Update(new Pixel {Value = ".", X = _x, Y = _y});
                    _x = nextX;
                    _y = nextY;
                    UpdateDroidPos();
                    break;
                case 2:
                    _screen.Update(new Pixel {Value = "O", X = _x, Y = _y});
                    _computer.Stop();
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(OutputCallback));
            }
        ...
        private (long x, long y) GetNextPosition()
        {
            return GetNextPosition(_dir);
        }
        
        private (long x, long y) GetNextPosition(long dir)
        {
            var (xD, yD) = dir switch
            {
                1 => (0L, -1L),
                2 => (0L, 1L),
                3 => (-1L, 0L),
        ...
        private void UpdateDroidPos()
        {
            _screen.Update(new Pixel {Value = "D", X = _x, Y = _y});
        }
